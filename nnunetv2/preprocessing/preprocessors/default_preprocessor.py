...
class DefaultPreprocessor(object):
    ...
    def run_case_npy(self, data: np.ndarray, seg: Union[np.ndarray, None], properties: dict,
        ...
        data = torch.from_numpy(data)#.to(gpu_device)
        data = data.to(torch.float32)
        data = torch.unsqueeze(data, 0)
        data = F.interpolate(data, size=tuple(new_shape), mode='area', antialias=False)
        data = torch.squeeze(data, 0).cpu().numpy().astype(np.float32)

        seg = torch.from_numpy(seg)#.to(gpu_device)
        seg = seg.to(torch.float32)
        seg = torch.unsqueeze(seg, 0)
        seg = F.interpolate(seg, size=tuple(new_shape), mode='area', antialias=False)
        seg = torch.squeeze(seg, 0).cpu().numpy()
